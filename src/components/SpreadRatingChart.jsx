...
const SERIES_CONFIG = [
  ...
  { key: 'aa1', label: 'Aa1', color: '#38bdf8' },
  { key: 'aa3', label: 'Aa3', color: '#1d4ed8' },
  { key: 'a2', label: 'A2', color: '#f97316' },
  { key: 'baa2', label: 'Baa2', color: '#6b7280' },
  { key: 'ba2', label: 'Ba2', color: '#d97706' },
]

const RATING_KEYS = SERIES_CONFIG.map((serie) => serie.key)
...
export default function SpreadRatingChart() {
  ...
  useEffect(() => {
    ...
    const margin = { top: 72, right: 112, bottom: 68, left: 80 }
    ...
    g.append('text')
      ...
      .text("Spreads por rating Moody's (bp)")
    ...
    seriesGroups
      ...
      .attr('y', (d) => {
        const last = [...d.values].reverse().find((v) => v.value != null && Number.isFinite(v.value))
        return last ? y(last.value) : y(maxValue)
      })
      .attr('fill', (d) => d.color)
      .attr('font-size', 11)
      .attr('font-weight', 600)
    ...
    const showTooltip = (event) => {
      ...
      seriesValueMaps.forEach((series, idx) => {
        ...
        if (dot) {
          d3.select(dot)
            .attr('cx', xPos)
            .attr('cy', value != null && Number.isFinite(value) ? y(value) : y(minValue))
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3 mb-4">
        <div>
          <p className="text-xs uppercase tracking-wide text-slate-500 font-semibold">Spread Rating</p>
          <p className="text-sm text-slate-600">
            Spreads por calificación Moody's (Aaa–Ba2) expresados en puntos básicos; permite seguir el diferencial entre
            segmentos de riesgo.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <>
          <svg ref={svgRef} className="w-full" role="img" aria-label="Gráfico de Spread por Rating" />
          <div
            ref={tooltipRef}
          ...
          <div className="mt-4 text-xs text-slate-500">
            <span className="text-slate-600">Nota:</span> se destacan las curvas Aaa, Aa1, Aa3, A2, Baa2 y Ba2. Ajusta el rango
            temporal para ver el histórico completo (5Y), los últimos 3 años o el último año.
          </div>
        </>
      )}
    </div>
